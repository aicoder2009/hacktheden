...
export function NavLinks({ links, exact = [] }: { links: { href: string; label: string }[]; exact?: string[] }) {
      ...
      {links.map((l) => {
        ...
        return (
          <Link
            ...
            href={l.href}
            className={cn(
              "shrink-0 px-2 py-1 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground",
